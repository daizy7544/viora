...
function oldPriceOf(p) {
  return Number(p.original_price ?? p.old_price) || 0;
}
...
async function loadProductsFromFirebase() {
  try {
    ...
    const snapshot = await fb.firestore.getDocs(
      fb.firestore.query(
        fb.firestore.collection(fb.db, "products"),
        fb.firestore.where("is_active", "==", true)
      )
    );
    ...
    console.error(
      "Viora Finds: Could not load products from Firebase.",
      error
    );

    /*
      Fallback to old localStorage data.
      This keeps the site usable while Firebase is being configured.
    */

    try {
      products = JSON.parse(
        localStorage.getItem("vf_products") || "[]"
...
async function loadAdsFromFirebase() {
  try {
    ...
    console.error(
      "Viora Finds: Could not load advertisements from Firebase.",
      error
    );

    try {
      ads = JSON.parse(
        localStorage.getItem("vf_ads") || "[]"
      ).filter(
        a =>
          a.active !== false &&
          a.is_active !== false
...
async function loadStore() {
  const [productsOK, adsOK] = await Promise.all([
    ...
    cart.length
      ...
      : `
        <div class="empty-state">
          ...
          <p>
            Add a few finds and send
            your order straight to WhatsApp.
          </p>
    ...
    <div class="vf-detail-grid">
      ...
      <div class="vf-detail-media">
        ...
        ${
          p.image_url
            ...
            `
            : `
              <div class="vf-detail-placeholder">
                V
              </div>
            `
        }
      ...
      <div class="vf-detail-copy">
        ...
        ${
          p.is_deal
            ? `
              <div class="vf-deal-line">
                Today's deal · limited-time pricing
              </div>
            `
            : ""
        }
        ...
        <div class="vf-microcopy">
          Choose a quantity, add it to your bag,
          or order this item directly.
        </div>
  ...
  ?.addEventListener(
    ...
    e => {
      ...
      if (!number) {
        ...
        alert(
          "Please set your WhatsApp number in config.js first."
        );
      ...
      const message = [
        ...
        ...items.map(
          (p, i) =>
            `${i + 1}. ${p.name} × ${Number(p.qty) || 1} — ${money(
              priceOf(p) *
              (Number(p.qty) || 1)
            )}`
        ),
